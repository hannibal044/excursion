...
bot = telebot.TeleBot('5606959452:AAGlXbT1UvAWr8fZm7lOIt3u2DxjYrELoLU')
language = "ru"
...
exhibit_dict = {'12': ['Граната РГД-13', 'https://disk.yandex.ru/d/NQjiV0B5fvGlCA', 'РГД-33 — советская ручная граната, \
               ...
               '18':  ['Автограф Константина Симонова, с его прикрепленными фотографиями', 'https://wampi.ru/image/RJYMoR0',
...
def handle_text(message):
    if message.data == 'h':
        ...
        bot.edit_message_text(chat_id=message.message.chat.id, message_id=message.message.message_id, text="Выберите героя, о котором хотите узнать",
        ...
        result = wikipedia.search(f"{(heroes_dict[message.data][0]).split()[0]}, {(heroes_dict[message.data][0]).split()[1]} {(heroes_dict[message.data][0]).split()[2]}")
